...
namespace talawa {
namespace core {
#define THROW_MATRIX_ERROR(msg) THROW_talawa_ERROR("Matrix", msg)
...
class Matrix {
 public:
  ...
  T item() const {
    if (data.size() != 1) {
      throw std::runtime_error("item() called on a Matrix with " +
                               std::to_string(data.size()) +
                               " elements. Expecting 1.");
    }
    return static_cast<T>(data[0]);
  }

  // Multiply this (A) with B^T (B passed as already transposed matrix)
  // A: (n x k), B_T: (m x k) where m = number of output cols
  // Result: (n x m)
  Matrix dotWithBTransposed(const Matrix& B_T) const;
  ...
  // Profiling: cumulative time spent in high-performance dot operations
  static double profiling_dot_time;
  ...
  void forEach(std::function<void(int row, int col, float)> func) const {
    for (size_t i = 0; i < rows; ++i) {
      for (size_t j = 0; j < cols; ++j) {
        func(i, j, (*this)(i, j));
      }
